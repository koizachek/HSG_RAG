...
- [ ] **Test the alert chain once**: temporarily change a price in `data/database/programme_facts.json` → `python -m src.pipeline.update_programme_facts` → email/Slack alert must arrive → revert the change
...
- **Never hardcode facts** — prices/deadlines/start dates come exclusively from `data/database/programme_facts.json` (auto-generated). Prompt changes containing numbers get rejected in review.
