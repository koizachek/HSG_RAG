...
class WeaviateService:
    ...
    def _init_client(self) -> wvt.WeaviateClient:
        ...
        if retries == 3:
            logger.error(f"Failed to establish a connection with the {self._connection_type} weaviate database after 3 retries!")
    ...
    def _select_collection(self, client, lang: str) -> tuple[Collection, str]:
        """
    ...
    def batch_import(self, data_rows: list, lang: str) -> list:
        ...
        with self._init_client() as client:
            collection, collection_name = self._select_collection(client, lang)
