...
def test_agent_chain_generates_session_id_when_missing(monkeypatch):
    ...
    assert agent.state_manager.conversation_state["session_id"] == agent._user_id
    assert agent.state_manager.conversation_state["user_id"] == agent._user_id
...
def test_log_user_profile_uses_valid_session_path(monkeypatch, tmp_path):
    ...
    agent.state_manager.log_user_profile()
